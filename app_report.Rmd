...
ss2=sprintf("SELECT from_unixtime(date/1000,'%%Y-%%m-%%d') AS 'DATE',`name` ,min(pm) from iso_rank where from_unixtime(date/1000,'%%Y-%%m-%%d')>'%s' GROUP BY from_unixtime(date/1000,'%%Y-%%m-%%d'),`name` ",as.Date(yesterday-7))
...
ss2.4 = sprintf("SELECT pm AS \"排名\",app_name as \"名称\"  FROM aso100 where date=\"%s\" limit 30 ",as.Date(yesterday))
datax2.4= dbGetQuery(conn,ss2.4)
datax$name=''
...
data2=data_rank[which(data_rank$DATE==yesterday),]



```
<font size=3><B>基本情况</B><br></font>
1.至昨日总累计用户 ：`r  data1$lj_yh[2]+data1$lj_yh[1]`人。<br>
2.昨日新增用户 `r  sum(data1$xz_yh)`人、日活跃用户 `r sum(data1$hy_yh)`人。<br>
3.IOS中国娱乐榜畅销排名为`r data2[which(data2$name=="娱乐(畅销)"),3]` 名、下载排名为`r data2[which(data2$name=="娱乐(免费)"),3]`名。<br>

分析：<br>


<font size=3><B>昨日app直播浏览前三名：</B></font><br>

<font size=3><B>app各大板块的浏览点击信息：</B></font><br>


<font size=2><B>详细情况请看附件EXCEL表格</B><br></font>

#<font size=4.5>1.近7日iOS平台中国娱乐分类内下载排名(无排名则不上榜、无数据)</font><br>
```{r,echo=FALSE}
datax01=data_rank[which(data_rank$name=='娱乐(免费)'),][1:7,]
plot(1:7,datax01$`min(pm)`,type = "b",lty=2,pch=20,axes = F,xlab = "日期",ylab = "排名",col="#5B9BD5",ylim = c(max(datax01$`min(pm)`,na.rm = T)%/%100*100+100,min(datax01$`min(pm)`,na.rm = T)%/%100*100-100))
axis(1,at=1:7,labels = c(format(as.Date(datax01$DATE), "%m/%d")))
axis(2,at=seq(max(datax01$`min(pm)`,na.rm = T)%/%100*100+100,min(datax01$`min(pm)`,na.rm = T)%/%100*100,-100),las=2)
text(1:7,datax01$`min(pm)`,datax01$`min(pm)`,pos = 3)
```
<br>

#<font size=4.5>2.近7日iOS平台中国娱乐分类内畅销排名(无排名则不上榜、无数据)</font><br>
```{r,echo=FALSE}
datax01=data_rank[which(data_rank$name=='娱乐(畅销)'),][1:7,]
plot(1:7,datax01$`min(pm)`,type = "b",lty=2,pch=20,axes = F,xlab = "日期",ylab = "排名",col="#5B9BD5",ylim = c(max(datax01$`min(pm)`,na.rm = T)%/%100*100+100,min(datax01$`min(pm)`,na.rm = T)%/%100*100-100))
axis(1,at=1:7,labels = c(format(as.Date(datax01$DATE), "%m/%d")))
axis(2,at=seq(max(datax01$`min(pm)`,na.rm = T)%/%100*100+100,min(datax01$`min(pm)`,na.rm = T)%/%100*100,-50),las=2)
text(1:7,datax01$`min(pm)`,datax01$`min(pm)`,pos = 3)
```

#<font size=4.5>3.最近七日新增用户（Andriod与iOS平台）</font><br>
```{r,echo=FALSE}
...
#<font size=4.5>4.最近七日活跃用户</font><br>
```{r, echo=FALSE}
...
legend(10.4,max(active_user)*1.7,pt.bg=c("#5B9BD5","#ED7D31"),c("andriod","ios"),pch = 22,pt.cex = 2)
```
<br>
...
#2<font size=4.5>近7日iOS平台中国娱乐分类内畅销排名(无排名则不上榜、无数据)</font><br>
```{r,echo=FALSE}
datax01=data_rank[which(data_rank$name=='娱乐(畅销)'),][1:7,]
plot(1:7,datax01$`min(pm)`,type = "b",lty=2,pch=20,axes = F,xlab = "日期",ylab = "排名",col="#5B9BD5",ylim = c(max(datax01$`min(pm)`,na.rm = T)%/%100*100+100,min(datax01$`min(pm)`,na.rm = T)%/%100*100-100))
axis(1,at=1:7,labels = c(format(as.Date(datax01$DATE), "%m/%d")))
...
text(1:7,datax01$`min(pm)`,datax01$`min(pm)`,pos = 3)
```
<br><font size=3.5>畅销排在粉丝网APP前面的列表（或前30名）：</font><br>
```{r,echo=FALSE}
print(datax2.4[,c(1,2)])
```
...
legend(13,max(datax6.1)*1,pt.bg=c("#5B9BD5","#ED7D31"),c("新用户","老用户"),pch = 22,pt.cex = 2)
text(labels=name6.1,x=(xx6[2,]+xx6[1,])/2,y=-max(datax6.1)*0.17,srt=65,cex = 0.8)
axis(2,at=seq(0,max(datax6.1)*1.3,200),labels =seq(0,max(datax6.1)*1.3,200) ,cex.axis=0.8,las=2)
```
...
lines(xx7[,1],datax7.1*100+5000,type = "b",col="#ED7D31",lwd=2,cex=1,pch=15)
text(xx7[,1],(datax7.1*100+5000),labels = paste(datax7.1,"%"),pos=3,cex = 0.7)
axis(side = 4,at=seq(5000,7000,1000),labels =paste((seq(5000,7000,1000)-5000)/100,"%"),cex.axis=0.7 ,las=2)
...
lines(cishu_andriod_t,cishu_andriod,type = "b",pch=15,lty=2,col="#5B9BD5",lwd=2,cex=1)
 ...
 legend(1.5,max(cishu_iOS,cishu_andriod)-50,c("Andriod","iOS"),pch = c(15,17),col = c("#5B9BD5","#ED7D31"))
 axis(side = 1,at=(0:24)-0.5,labels =0:24,cex.axis=0.73)
 axis(side=2,at=seq(0,max(max(cishu_iOS,cishu_andriod))+300,by=200),labels =seq(0,max(max(cishu_iOS,cishu_andriod))+300,by=200),cex.axis=0.85,las=2)
 abline(h=c(seq(0,max(max(cishu_iOS,cishu_andriod))+300,200)),lty=15,lwd=0.1)
```
