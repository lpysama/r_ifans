...
axis(2,at=seq(max(datax02$rank,na.rm = T)%/%10*10+10,min(datax02$rank,na.rm = T)%/%10*10-10,-50),las=2)
...
legend(6.8,max(active_user)*1.4,pt.bg=c("#5B9BD5","#ED7D31"),c("andriod","ios"),pch = 22,pt.cex = 2)
```
<br>
...
lines(1:7,rate_andriod,type = "b",pch=15,lty=2,col="#5B9BD5",lwd=2,cex=1.5)
 ...
 legend(6,min(rate_andriod,rate_iOS)*5,c("Andriod","iOS"),pch = c(15,17),col = c("#5B9BD5","#ED7D31"))
...
lines(cishu_andriod_t,cishu_andriod,type = "b",pch=15,lty=2,col="#5B9BD5",lwd=2,cex=1)
 ...
 axis(side=2,at=seq(0,max(max(cishu_iOS,cishu_andriod))+200,by=50),labels =seq(0,max(max(cishu_iOS,cishu_andriod))+200,by=50),cex.axis=0.85,las=2)
 abline(h=c(seq(0,max(max(cishu_iOS,cishu_andriod))+200,50)),lty=15,lwd=0.1)
```
