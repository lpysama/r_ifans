...
axis(2,at=seq(max(datax01$rank,na.rm = T)%/%100*100+100,min(datax01$rank,na.rm = T)%/%100*100,-100),las=2)
...
plot(1:7,datax02$rank,type = "b",lty=2,pch=20,axes = F,xlab = "日期",ylab = "排名",col="#5B9BD5",ylim = c(max(datax02$rank,na.rm = T)%/%10*10+20,min(datax02$rank,na.rm = T)%/%10*10-20))
axis(1,at=1:7,labels = c(format(as.Date(datax02$date), "%m/%d")))
axis(2,at=seq(max(datax02$rank,na.rm = T)%/%10*10+20,min(datax02$rank,na.rm = T)%/%10*10-20,-50),las=2)
